...
void simpleTest(flexiv::Robot* robot, flexiv::Log* log, flexiv::RobotStates& robotStates,int counter)
{
    //double test_value = 0;
            ...
            if (motionType == "hold") {
                ...
                targetTcpPose[1] = initTcpPose[1]
                                   + k_swingAmp
                                         * sin(2 * M_PI * k_swingFreq
                                               * loopCounter * k_loopPeriod);
                targetTcpPose[2] = initTcpPose[2]
                                    + k_swingAmp
                                        * cos(2 * M_PI * k_swingFreq
                                            * loopCounter * k_loopPeriod);
                robot->streamTcpPose(targetTcpPose);
                printf("%d\n",loopCounter);
            } else {
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        while (counter < 99999){
            simpleTest(&robot, &log, robotStates, counter);
            counter++;
        }
